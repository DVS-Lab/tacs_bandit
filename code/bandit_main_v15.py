...
class LSLStimulationTrigger:
    ...
    def wait_for_stimulation_start(self, timeout: float = None) -> bool:
        ...
        print("LSL: Waiting for stimulation ramp-up...")
        ...
        while True:
            try:
                ...
                # Show periodic status (every 10 seconds, only once per interval)
...
class TwoArmedBanditTask:
    ...
    def setup_display(self):
        """Setup Pygame display with multi-monitor support"""
        # Detect available displays
        num_displays = pygame.display.get_num_displays()
        print(f"Detected {num_displays} display(s)")
        
        # Choose target display (prefer second monitor if available)
        target_display = 1 if num_displays > 1 else 0
        if target_display > 0:
            print(f"Using secondary display (Display {target_display + 1})")
        else:
            print("Using primary display")
        
        try:
            if self.config['display']['fullscreen'] or num_displays > 1:
                # Use fullscreen on target display
                if target_display > 0:
                    # Get display info for secondary monitor
                    display_info = pygame.display.get_desktop_sizes()
                    if len(display_info) > target_display:
                        self.width, self.height = display_info[target_display]
                        # Position window on target display
                        os.environ['SDL_VIDEO_WINDOW_POS'] = f"{sum(display_info[i][0] for i in range(target_display))},0"
                    else:
                        self.width, self.height = display_info[0]
                else:
                    self.width, self.height = pygame.display.get_desktop_sizes()[0]
                
                self.screen = pygame.display.set_mode((self.width, self.height), pygame.FULLSCREEN)
                print(f"Fullscreen mode: {self.width}x{self.height}")
                ...
                print(f"Windowed mode: {self.width}x{self.height}")
                
        except Exception as e:
            print(f"Display setup failed: {e}")
            print("Falling back to primary display in windowed mode")
            # Fallback to primary display
            self.width = self.config['display'].get('window_width', 1024)
            self.height = self.config['display'].get('window_height', 768)
            self.screen = pygame.display.set_mode((self.width, self.height), pygame.SHOWN)
    ...
    def show_slots(self, highlight=None):
        """Show the two flower slot machines with circular highlights"""
        self.screen.fill(self.BLACK)
        ...
        if self.current_flowers[0] in self.flower_images:
            ...
            if highlight == 1:
                # Draw circular highlight around flower
                center = self.slot1_rect.center
                pygame.draw.circle(self.screen, self.WHITE, center, self.slot_size // 2 + 5, 3)
        ...
        if self.current_flowers[1] in self.flower_images:
            ...
            if highlight == 2:
                # Draw circular highlight around flower
                center = self.slot2_rect.center
                pygame.draw.circle(self.screen, self.WHITE, center, self.slot_size // 2 + 5, 3)
        
        pygame.display.flip()
    
    def get_response(self, max_time):
        """Get keyboard response only during active stimulus presentation"""
        start_time = time.time()
        clock = pygame.time.Clock()
        
        # Clear any pending events to prevent accidental responses
        pygame.event.clear()
    ...
    def run(self):
        ...
        try:
            ...
            # Show extended initial fixation to help participants orient
            print("Showing initial fixation...")
            self.show_fixation(3.0)  # 3-second initial fixation
