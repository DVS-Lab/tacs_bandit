...
class LSLStimulationTrigger:
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            # CHANGE #8: Suppress verbose network interface output from pylsl
...
class TwoArmedBanditTask:
    ...
    def get_subject_info(self):
        ...
        # Setup counterbalancing (for internal tracking only)
        if self.stimulation_enabled and self.stimulation_manager:
            ...
            # CHANGE #2: Remove counterbalance printout
            # (Silently set up counterbalancing without printing)
    ...
    def show_waiting_screen(self):
        ...
        # Determine stim condition for display purposes only
        run_types = self.config['experiment']['run_types']
        run_type = run_types.get(str(self.run_number), 'unknown')
        
        # Get stimulation condition
        if self.stimulation_enabled and self.stimulation_manager:
            stim_condition = self.stimulation_manager.get_run_condition(self.run_number)
        else:
            subject_num = int(self.subject_info['subject_id']) if self.subject_info['subject_id'].isdigit() else 0
        ...
        # CHANGE #1 & #3: Removed stim condition display, updated text
        if stim_condition == 'baseline':
            waiting_text = [
                f"Two-Armed Bandit Task - Run {self.run_number}",
                "",
                "No stimulation for this run",
                "",
                "The experimenter will start the task momentarily"
            ]
        else:
            # Don't show which protocol (active/sham) - double blind
            waiting_text = [
                f"Two-Armed Bandit Task - Run {self.run_number}",
                "",
    ...
    def show_instructions(self):
        ...
        instructions = [
            ...
            "",
            # CHANGE #3: Updated instruction text
            "The experimenter will start the task momentarily"
        ]
        ...
        # Don't wait for space - let experimenter control timing
        # (Instructions will be replaced by waiting screen or task start)
    ...
    def run_single_run(self):
        ...
        time.sleep(2)  # Brief pause to read instructions
        
        # Show waiting screen and wait for trigger
