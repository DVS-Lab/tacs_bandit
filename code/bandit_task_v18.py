...
class TwoArmedBanditTask:
    ...
    def setup_display(self):
        ...
        if self.config['display']['fullscreen']:
            # Get desktop sizes for each display
            desktop_sizes = pygame.display.get_desktop_sizes()
            
            if self.display_choice < len(desktop_sizes):
                # Get size of selected display
                display_width, display_height = desktop_sizes[self.display_choice]
                
                # Calculate position for the selected display
                x_offset = sum(desktop_sizes[i][0] for i in range(self.display_choice))
                
                # Set window position before creating display
                os.environ['SDL_VIDEO_WINDOW_POS'] = f"{x_offset},0"
                
                # Create fullscreen window on selected display
                # Use NOFRAME to avoid capturing all displays
                ...
                # Fallback to default fullscreen
            ...
            if self.display_choice < len(desktop_sizes):
                x_offset = sum(desktop_sizes[i][0] for i in range(self.display_choice))
                os.environ['SDL_VIDEO_WINDOW_POS'] = f"{x_offset + 50},50"
